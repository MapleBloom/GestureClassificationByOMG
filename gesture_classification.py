...
def main():
    ...
    while True:
        try:
            print(f"{bcolors.OKG}\nOMG sensors data should be stored in 'X_test.npy' compressed to .zip file or not and contain 40 sensors data.")
            print(f"X_test structure: np.array( number of samples, number of sensors (=40) , number of time ticks in one sample )")
            inp = input(f"{bcolors.OKB}\nEnter path to .zip with OMG sensors data or press ENTER for default path 'data/X_test.zip': ")
            if inp == '':
                inp = 'data/X_test.zip'
            ...
            inp = input(f"{bcolors.FL}Path to the file should be a string. Press any key to continue or 'q' to exit: ")
        except FileNotFoundError:
            inp = input(f"{bcolors.FL}File 'X_test.npy' at this path doesn't exist. Press any key to continue or 'q' to exit: ")
        finally:
            check_exit(inp)

    TS_SAMP = X_test.shape[0]  # samples in the test data
    TS_TICKS = X_test.shape[2]  # time ticks at one sample
    assert SENS == X_test.shape[1], f'{bcolors.FL}Test data should contain {SENS} OMG sensors{bcolors.ENDC} ' # check number of sensors in the test data
    ...
    while True:
        ...
        if i == TS_SAMP:
            ...
            exit()
        inp = input(f"{bcolors.OKB}Press any key to continue or 'q' to exit: ")
        check_exit(inp)
